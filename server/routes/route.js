    ...
    try
    {
        ...
        if (userLogin)
        {
            ...
            else if (isMatch && userLogin.Role == "1")
            {
                ...
                const token = jwt.sign({ id: userLogin._id }, process.env.SecretKey, { expiresIn: "5 seconds" });
                console.log(token)
...
router.post("/sign-up", async (req, res) =>
{
    ...
    try
    {
        ...
        if (!Name || !Email || !Password || !CPassword) return res.status(404).json({ message: "All Field are Required" })

        const userEmail = await loginuser.findOne({ Email })
        if (userEmail) return res.status(400).json({ message: "User Already Exist" })

        if (Password !== CPassword) return res.status(400).json({ message: "Password Not Matching" });
...
router.post("/admin/movies", upload.single('photo'), async (req, res) =>
{
    try
    {
        ...
        if (!files) return res.status(400).json({ message: "Please Select the Poster Image" });
